...
int strkvm_parse(st_strkvm *strkvm, char *buffer, size_t size)
{
  char *buf = NULL;
  char *end, *begin;
  int result = 0;
...
int strkvm_tostring(st_strkvm *strkvm, char **buffer, size_t *length)
{
  FILE *out = NULL;
  st_strkv *var = NULL;
  ...
  for (var = strkvm->vars; var; var = var->next) {
    if (var->value != NULL)
      fprintf(out, "%s=%s;", var->name, var->value);
    else
